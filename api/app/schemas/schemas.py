...
class ChatLog(BaseModel):
  id: int = Field(..., title="チャットID", description="チャットを一意に識別するためのID")
  ...
  session_id: int = Field(..., title="セッションID", description="関連するセッションのID")
  class Config:
    schema_extra = {
      "example": {
          "id": 1,
...
class Sessions(BaseModel):
  id: int = Field(..., title="セッションID", description="セッションを一意に識別するためのID")
...
class ErrorLog(BaseModel):
  id: int = Field(..., title="エラーログID", description="エラーログを一意に識別するためのID")
